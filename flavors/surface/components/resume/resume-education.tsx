      ...
      {groupEducation(entries).map((group) => (
        ...
        >
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            ...
            <p className="text-[0.9375rem] text-[#4b4944]">{group.location}</p>
          </div>
          <ul className="mt-1 space-y-0.5 print:mt-0.5">
            {group.entries.map((entry) => (
              ...
              >
                <p className="text-[0.9375rem] text-[#4b4944]">
                  {[entry.degree, entry.score].filter(Boolean).join(" · ")}
                </p>
                <p className="font-display text-[0.75rem] tracking-[0.08em] whitespace-nowrap text-[#6f6c65] tabular-nums">
