import { Disclosure } from "@/flavors/timetable/components/ui";
...
/**
 * One notice: the opening question, its published replies joined by the
...
export function ChatThread({
  ...
  thread: Question;
  /** `Notice 014`; omitted for the sender's own not-yet-published echo. */
  label?: string;
  ...
  return (
    <article className="min-w-0">
      {label && (
        <p className="mb-3 flex flex-wrap items-center gap-2.5 font-mono text-mono-sm font-bold tracking-[0.08em] text-ink-soft uppercase">
          <span className="text-ink">{label}</span>
          <span
            className={
              answered
                ? "rounded-[3px] bg-ink px-1.5 pt-1 pb-0.5 leading-none text-ground"
                : "rounded-[3px] bg-signal px-1.5 pt-1 pb-0.5 leading-none text-signal-ink"
            }
          >
        ...
        {standalone || thread.replies.length === 0 ? (
          ...
          <li className={threadItemClass}>
            <Disclosure
              ...
              summaryClassName="ml-5 min-h-11 w-fit items-center gap-1.5 rounded-sm font-mono text-mono-sm font-bold text-ink-soft transition-colors hover:text-ink sm:ml-7"
