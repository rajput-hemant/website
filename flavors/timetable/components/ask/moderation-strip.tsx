...
import { Button, IconButton, Tag } from "@/flavors/timetable/components/ui";
...
/** Owner-only queue of pending and recently flagged messages, above the notices. */
...
function ModerationQueue({ className }: { className?: string }) {
  ...
  return (
    <section
      ...
      data-print="hide"
      className={cn(
        "rounded-md border border-rule-strong bg-surface/60",
        className
      )}
    >
      <header className="flex items-center justify-between gap-4 border-b border-rule-strong py-2 pr-2 pl-4">
        <h2
          id={headingId}
      ...
      {queue.state === "ready" && queue.items.length > 0 && (
        <ol className="divide-y divide-rule-strong">
