...
/**
 * The card's content: a 16:9 image slot (reserved before the image loads, so
 * nothing jumps), the title, a two-line description and the domain. With no
...
export function LinkPreviewCard({ href, preview }: LinkPreviewCardProps) {
  ...
  return (
    <div className="w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded-lg border border-border bg-background shadow-popover">
      {preview.image && (
        <div className="relative aspect-video overflow-hidden border-b border-border bg-surface">
          {status === "failed" ? (
            <span className="absolute inset-0 grid place-items-center font-mono text-2xs text-subtle">
              {displayDomain(href)}
            </span>
          ) : (
