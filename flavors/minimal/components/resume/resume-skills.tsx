 ...
 * two-column ruled list. Print keeps the compact label rows.
 */
export function ResumeSkills({ groups }: { groups: SkillGroup[] }) {
  return (
    <dl className="grid gap-y-2 text-[0.9375rem] md:block md:columns-2 md:gap-x-8 print:grid print:columns-auto print:gap-y-2">
      {groups.map((group) => (
        <div
          key={group.id}
        ...
        >
          <dt className="font-semibold text-foreground md:text-sm">
            {group.title}
          </dt>
          <dd className="mb-2 text-muted sm:mb-0 md:mt-1 print:mt-0">
            {group.items.join(", ")}
          </dd>
        </div>
      ))}
