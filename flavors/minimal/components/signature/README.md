...
3. Replace the contents of `signature-paths.ts` (and stop running `bun run signature`, which would overwrite it):
   ...
   - `signatureStrokes`: one `{ d, duration, pause }` per stroke. `duration` is the time in ms the pen takes (keep it proportional to the stroke's length); `pause` is the ms the pen is lifted before the stroke; keep it at 0 so the signature writes in one continuous motion. Aim for 1.6–2.4 s in total.
