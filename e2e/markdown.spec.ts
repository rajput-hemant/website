...
test.describe("sitemap.xml", () => {
  test("is a urlset with every page", async ({ request }) => {
    ...
    // Crawlers get the default edition, so pages it redirects away from are left out.
    for (const page of pages) {
      const listed = !("only" in page) || page.only === DEFAULT_FLAVOR;
      expect(locs.includes(page.path)).toBe(listed);
    }
