...
- `(site)` and `(studio)` are separate root layouts, so unmatched URLs only reach `(site)/not-found.tsx` through the `(site)/[...slug]` catch-all that calls `notFound()`. Keep it when adding routes.
